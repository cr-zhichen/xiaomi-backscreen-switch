...
- 打开 App 时自动检查更新，首页底部可查看当前版本、手动检查并下载新版本。
...
自动更新检查每 6 小时最多进行一次，仅访问本项目的公开 GitHub Releases，不需要 GitHub 登录或 Shizuku 授权。发现新版本后，底部提示可打开独立的更新弹窗；网络失败不会影响背屏操作，也可点击首页底部的“检查更新”随时手动重试。下载通过系统浏览器完成，安装由用户确认。

正式版只接收更新的正式版本；`-beat.1`、`-beta.1` 等测试版也会检查更新的预发布版本，并可升级到同版本的正式版。版本按数字比较，不会将 `1.0.9` 当成比 `1.0.10` 更新的版本。
